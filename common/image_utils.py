...
def union_rbox(result, alpha=0.1):
    ...
    def sort_group_box(boxes):
        ...
        char_text = ''.join([bx['text'] for bx in boxes])

        all_chars_list = []

        box4 = np.zeros((N, 8))
        for i in range(N):
            chars_list = boxes[i]['chars_list']
            if chars_list != None:
        ...
        bboxes = np.array(points, dtype="float32")
        score_lst = [float(bx['score']) for bx in boxes]
        ave_score = np.mean(score_lst)

        union_result = {'text': text, 'origin_bboxes': box4.tolist(), 'bboxes': bboxes,
                        'char_text':char_text, 'chars_list': all_chars_list, 'score': ave_score}
        # if len(all_chars_list) == 0:
        return union_result
        # else:
            # return union_result.update({'chars_list': all_chars_list})
...
def img_process(image):
    ...
    RedThresh =cv2.adaptiveThreshold(R_channel, 255, cv2.ADAPTIVE_THRESH_MEAN_C,cv2.THRESH_BINARY, 75, 45)
    RedThresh = cv2.blur(RedThresh, (3, 3))
    return RedThresh